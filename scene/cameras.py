...
class Camera(nn.Module):
    ...
    def inmat(self):
        ...
        inmat = np.eye(3,dtype=np.float32)
        inmat[[0,0,1,1],[0,2,1,2]] = np.array([focal_x,self.image_width*0.5,focal_y,self.image_height*0.5])
        # inmat = torch.tensor(inmat).to(self.world_view_transform.device)
        return inmat
    @property
    def rvec(self):
        R = self.R.T
