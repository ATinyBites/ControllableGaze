...
class FreqEncoding(nn.Module):
    ...
    def forward(
        ...
        """
        x = (in_tensor + 1.0)/2.0
        scaled_in_tensor = 2 * torch.pi * x  # scale to [0, 2pi]
        ...
        scaled_inputs = scaled_inputs.view(scaled_inputs.shape[0],scaled_in_tensor.shape[1], -1)  # [..., "input_dim" * "num_scales"]
        encoded_inputs = torch.sin(torch.cat([scaled_inputs, scaled_inputs + torch.pi / 2.0], dim=-1)).flatten(1)
...
class GaussianModel(torch.nn.Module): 
    ...
    def __init__(self, sh_degree : int):
        ...
        self._features_dynamic = torch.empty(0)
        ...
        self.ColorDecoder = nn.Sequential(nn.Linear(self.dynamic_dim+9*3,64),
                        nn.LeakyReLU(),
                        nn.Linear(64,64),
                        nn.LeakyReLU(),
                        nn.Linear(64,64),
    ...
    def get_rotation(self): 
        if self.binding is None:
            ...
            quat = quat_xyzw_to_wxyz(quat_product(quat_wxyz_to_xyzw(face_orien_quat), quat_wxyz_to_xyzw(rot)))
            return quat  # roma 
    ...
    def create_from_pcd(self, pcd : Optional[BasicPointCloud], spatial_lr_scale : float):
        ...
        self._features_dynamic = nn.Parameter(torch.zeros((len(self._xyz),self.dynamic_dim), device="cuda",dtype=torch.float).contiguous().requires_grad_(True))

    # def get_xyz(self):
    #     return self._xyz

    # def get_features(self):
    #     return torch.cat([self._features_dc, self._features_rest], dim=2).transpose(1, 2).contiguous()
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._features_dynamic], 'lr': training_args.feature_lr, "name": "f_dynamic"},
            {'params': list(self.ColorDecoder.parameters()),'lr':1e-4,"name":"decoder"}
    ...
    def load_ply(self, path, **kwargs):
        ...
        self._features_dynamic = nn.Parameter(torch.tensor(torch.zeros(len(xyz),self.dynamic_dim), dtype=torch.float, device="cuda").contiguous().requires_grad_(True))
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if group["name"] not in ["xyz","scaling","rotation","opacity","f_dc","f_rest","f_dynamic"]:
    ...
    def prune_points(self, mask):
        ...
        self._features_dynamic = optimizable_tensors["f_dynamic"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation,new_features_dynamic=None):
        d = {"xyz": new_xyz,
        ...
        "f_dynamic": new_features_dynamic
        }
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_features_dynamic = self._features_dynamic[selected_pts_mask].repeat(N,1)
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation,new_features_dynamic)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_features_dynamic = self._features_dynamic[selected_pts_mask]
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation,new_features_dynamic)
