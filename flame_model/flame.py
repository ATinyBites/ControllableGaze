...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        mp_lmk_embeddings = np.load('flame_model/assets/flame/mediapipe_landmark_embedding.npz')
        self.register_buffer(
                    "mp_full_lmk_faces_idx",
                    torch.tensor(mp_lmk_embeddings["lmk_face_idx"].astype(np.int32), dtype=torch.long).reshape(1,-1),
                )
        self.register_buffer(
            "mp_full_lmk_bary_coords",
            torch.tensor(mp_lmk_embeddings["lmk_b_coords"], dtype=self.dtype).reshape(1,-1,3),
        )
    ...
    def forward(
        ...
        eyes,
        translation,
        zero_centered_at_root_node=True,  # otherwise, zero centered at the face,False
        use_rotation_limits = False, # False
